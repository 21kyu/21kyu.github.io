...
Go 스케줄러의 설계와 동작은 멀티스레드 기반 Go 프로그램이 더육 효율적면서 좋은 성능을 발휘할 수 있게 한다.
...
고려해야할 사항에는 다중 프로세서와 코어, [CPU 캐시 그리고 NUMA](https://frankdenneman.nl/2016/07/06/introduction-2016-numa-deep-dive-series/)
...
할당된 명령어 집합을 연산하고 순차적으로 실행하는 것은 스레드가 해야할 일이다.
...
운영체제 스케줄러는 실행되어야할 스레드가 있는 경우 코어가 유휴 상태가 아닌지 확인하는 역할을 한다.
...
스레드가 할 수 있는 작업엔 두 가지의 유형이 있는데 첫번째는 CPU 바운드라고 하고 두번째는 IO 바운드라 한다.
...
IO 바운드의 작업에 초점을 맞춘 프로그램에서의 컨텍스트 스위치는 이점이 있다.
